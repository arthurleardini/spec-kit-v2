...
2. A biblioteca de componentes **não deveria ser bespoke**: o produto usa uma lib
   canônica (tipo Material Angular — https://material.angular.dev/components/categories).
   O catálogo existe só para dar uma **referência canônica de como descrever telas**,
   não para inventar componentes próprios.

## Objetivos

- Cada tela ganha um wireframe **fat marker** renderizado no navigator, autorado em
  um bloco ` ```wireframe ` no próprio markdown.
- `componentes.md` vira um **ponteiro fino** para o Material Angular; telas referenciam
  `mat-*` direto. Os 11 blocos bespoke são removidos.
...
| # | Decisão |
|---|---------|
| D1 | `componentes.md` = **ponteiro fino** p/ Material Angular; telas citam `mat-*`. Remove os 11 blocos bespoke. |
...
| D5 | Prosa = **aumentar** (mantém Objetivo/Conteúdo/Ações/Estados/Navegação + sketch). `Blocos:` → `Componentes:` (mat-*). |
...
| Sintaxe | Nó | Material (anotação) |
|---|---|---|
| `# Texto` | barra de título (largura total) | `mat-toolbar` |
| `## Texto` | subtítulo / cabeçalho de seção | — |
| `[[ a \| b \| c ]]` | **linha** de N células iguais; cada célula é um token recursivo | grid / `mat-card` row |
| `card "Título":` + linhas indentadas | **card/região**; conteúdo = linhas filhas | `mat-card` |
| `[Rótulo____]` (≥2 underscores finais) | **input** | `mat-form-field` |
| `[~ legenda ~]` | **placeholder de gráfico** | (chart) |
| `[Texto]` (1+ por linha, sem underscores) | **botão(ões)** | `mat-button` |
| `(!) texto` | **alerta/banner** | banner |
| `- item` | **item de lista** (linhas `-` consecutivas = uma lista) | `mat-list` |
| `\| a \| b \|` (linhas consecutivas) | **tabela**; 1ª linha = cabeçalho | `mat-table` |
...
- **Componentes:** mat-toolbar, mat-card, mat-list, mat-button
```

`Blocos:` (bespoke) é renomeado p/ `Componentes:` listando `mat-*` do Material Angular.

## componentes.md (ponteiro fino)

Reescrito para:
- 1 parágrafo: "A lib de componentes é o **Material Angular**
  (https://material.angular.dev/components/categories). Telas referenciam os
  componentes pelo nome `mat-*`; o sketch usa a DSL `wireframe`. Este arquivo é só a
  **referência canônica de vocabulário** — não um catálogo próprio."
- Uma lista das **categorias** do Material (Form controls, Navigation, Layout,
  Buttons & indicators, Popups & modals, Data table) + o mapa DSL→`mat-*` (a tabela
  acima).
...
- `skills/contrato-telas-fluxos/SKILL.md` — instruir geração do bloco `wireframe`
  (DSL), o `Componentes:` mat-*, e a referência ao Material.
...
- Cada tela `T-NN` tem 1 bloco `wireframe` e linha `Componentes:` com `mat-*`.
