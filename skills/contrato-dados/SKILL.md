---
name: contrato-dados
description: Use quando o usuário quer gerar ou revisar o modelo de Dados (Cap. 4) do contrato.md de uma feature no modelo wikiLLM — modelo derivado do eixo, não paralelo.
---

# contrato-dados

Escreve e mantém o **Capítulo 4 — Dados** do documento único `refined/contracts/<feature>/contrato.md` da Camada de Contrato. Não cria arquivo solto: o contrato de cada feature é **um só** (4 capítulos); esta skill é dona do Cap. 4.
...
**Regra de não-sobreposição** (ver bloco-intro do `contrato.md`): cada fato mora em UM lugar. Este capítulo é o modelo conceitual; invariantes de domínio permanecem como `RN-NN` na Visão (citados por ID, nunca reescritos aqui), e o que o sistema deve fazer fica na tabela RF do Cap. 3.1.
...
Ler `eixo` no frontmatter do `contrato.md`. O Cap. 4 **deriva** do que já foi escrito nos capítulos anteriores:
...
## Entrada
A Camada de Intenção em `refined/intencao/visao.md` (especialmente o Glossário do Cap. 2 para o vocabulário e as Regras do Cap. 3), a entidade da feature em `refined/entities/features/<feature>.md` e os **Caps. 1, 2 e 3** já preenchidos no `contrato.md` da feature. Se a fonte for documento binário (PDF/DOCX/PPTX), converter com `markitdown` antes de ler.

## Processo
1. Ler `refined/intencao/visao.md` e a fonte da feature. Conferir o `eixo` e reler os Caps. 1–3 do `contrato.md` (deles vem o modelo).
2. No mesmo `refined/contracts/<feature>/contrato.md`, preencher o **Cap. 4 — Dados**, sem tocar nos Caps. 1, 2 e 3, **derivando** conforme o `eixo`:
...
## Saída
**Cap. 4** do `refined/contracts/<feature>/contrato.md` preenchido, com frontmatter `tipo: contract` — somente entidades, campos (tipos agnósticos) e relações, **derivados** do eixo da feature.
