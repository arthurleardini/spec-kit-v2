...
2. No mesmo `refined/Requisitos/<feature>.md`, preencher o **Cap. 2 — Requisitos & Cenários de Teste**, sem tocar nos Caps. 1 e 3:
   ...
   - **2.3 Requisitos Não-Funcionais:** apenas os RNF **próprios** da feature, agrupados por categoria (Performance, Segurança, Disponibilidade, Auditoria, Conformidade, Escalabilidade, Observabilidade); um marcador `RNF-<categoria>-NN` por requisito. Listar os `RNF-T-*` aplicáveis no bloco "Transversais aplicáveis"; só detalhar o que **diverge** ou **acrescenta** ao transversal. Manter o capítulo **enxuto e específico**: referenciar `RNF-T-*` por ID em vez de repetir a prosa do transversal.
     - **Linha de referência a Integrações (obrigatória):** a feature **não** traz mais a tabela de integrações. Em vez disso, uma única linha que referencia o **catálogo transversal**: `**Integrações:** ver [catálogo transversal](../requisitos-transversais.md) (§ Integrações) — esta feature usa: <SistemaA>, <SistemaB>, …`. Listar **só** os sistemas que a feature **realmente toca**, derivados dos fluxos (Cap. 1.1), telas (Cap. 1.2) e dados (Cap. 3), nomeados com o **vocabulário canônico** do Glossário (Cap. 2 de `visao.md`) — iguais aos da matriz do catálogo. Papel/direção/criticidade/fallback de cada sistema vivem no catálogo (seção `## Integrações` de `requisitos-transversais.md`), **não** se replicam aqui. Garantir que cada sistema citado exista no catálogo (se não existir, acrescentá-lo lá / sinalizar à skill `spec-transversais`). Se a feature não toca nenhum sistema externo, escrever "nenhuma integração externa".
