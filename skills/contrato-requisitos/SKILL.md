...
description: Use quando o usuário quer gerar ou revisar os Requisitos e Cenários de Teste (Cap. 3) do doc de Requisitos de uma feature no modelo wikiLLM.
---

# contrato-requisitos

Escreve e mantém o **Capítulo 3 — Requisitos & Cenários de Teste** do documento único `refined/Requisitos/<feature>.md` da Camada de Requisitos. Não cria arquivo solto: o doc de requisitos de cada feature é **um só** (4 capítulos); esta skill é dona do Cap. 3.
...
## Entrada
A Camada de Intenção em `refined/visao.md` (especialmente Regras `RN-NN` e Métricas `IM-NN` do Cap. 3), a fonte da feature (brainstorm/documento associado) e os **Caps. 1 e 2** já preenchidos no `Requisitos/<feature>.md` da feature. Se a fonte for documento binário (PDF/DOCX/PPTX), converter com `markitdown` antes de ler.

## Processo
1. Ler `refined/visao.md`, a fonte da feature e os Caps. 1 e 2 do `Requisitos/<feature>.md`. Conferir o `eixo`.
2. No mesmo `refined/Requisitos/<feature>.md`, preencher o **Cap. 3 — Requisitos & Cenários de Teste**, sem tocar nos Caps. 1, 2 e 4:
...
## Saída
**Cap. 3** do `refined/Requisitos/<feature>.md` preenchido, com frontmatter `tipo: contract`, RFs rastreáveis a RN/US, RNFs por categoria e cenários de teste `CT-NN` (Dado/Quando/Então) rastreáveis aos requisitos.
