...
## Quando usar
Crítica de procedência de um doc de feature. Roda depois dos outros críticos — depende
do texto já estabilizado.

## Entrada
- O doc de feature.
- `refined/visao.md` (`RN-*`, `RN-AI-*`, `JTBD-*`), `requisitos-transversais.md`
  (`RF-T-*`, `RNF-T-*`), `telas-comuns.md` (`A-*`).
...
1. **Rastreio falso — J-RAS-01.** Para cada par RF → RN citada: ler a RN na Visão e
   decidir se ela **justifica** aquele RF. Não justifica → achado, propondo a RN correta
   ou a criação da RN que falta na Visão.
2. **Fato duplicado — J-RAS-02.** Procurar o mesmo fato afirmado em dois lugares
   (feature × transversal, feature × Visão, feature × feature). Apontar qual é o lugar
   canônico pela regra de não-sobreposição do template e propor a citação por ID no outro.
3. **Sem fonte — J-RAS-03.** Afirmação de negócio (prazo, teto, competência, obrigação
   legal, nome de sistema) sem RN, sem `## Fontes` e sem marcador
   `⚠ NÃO IDENTIFICADO`. Achado: citar a fonte ou marcar a lacuna.
4. **Não-funcional.** Conferir que nenhum RNF é **definido** na feature: RNF vive só em
   `requisitos-transversais.md` e a feature cita por ID. O lint pega a definição local
